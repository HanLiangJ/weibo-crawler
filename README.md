...
该步骤非常必要，因为每次运行weibo.py都会向user_id_list.txt中写入设置参数，如果不清空该文件，下一次变更参数设置后，执行仍会按照第一次的设置进行。
...
用excel即可打开csv文件，我是直接打开就能正常显示，若出现显示问题可以网上查找解决方案。

此外，用户id可能为科学计数法，将单元格设置为常规即可。
